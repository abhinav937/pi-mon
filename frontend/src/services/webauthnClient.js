...
class WebAuthnClient {
  constructor(baseURL) {
    // Always same-origin to avoid mixed content
    const sameOrigin = `${window.location.protocol}//${window.location.host}`;
    this.baseURL = baseURL ? baseURL.replace(/\/$/, '') : sameOrigin;
  ...
  base64urlToBuffer(base64url) { const base64 = base64url.replace(/-/g, '+').replace(/_/g, '/'); const padLength = (4 - base64.length % 4) % 4; const padded = base64 + '='.repeat(padLength); const binary = atob(padded); const buffer = new ArrayBuffer(binary.length); const view = new Uint8Array(buffer); for (let i = 0; i < binary.length; i++) { view[i] = binary.charCodeAt(i); } return buffer; }
  bufferToBase64url(buffer) { const bytes = new Uint8Array(buffer); let binary = ''; for (let i = 0; i < bytes.length; i++) { binary += String.fromCharCode(bytes[i]); } const base64 = btoa(binary); return base64.replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, ''); }
  ...
  async logout() {
    try {
      if (!this.token) return { success: true, message: 'Already logged out' };
      const response = await fetch(`${this.baseURL}/api/auth/logout`, { method: 'POST', headers: { 'Authorization': `Bearer ${this.token}`, 'Content-Type': 'application/json' } });
      const result = await response.json();
      this.token = null; localStorage.removeItem('webauthn-token');
      return { success: true, message: result.message || 'Logged out successfully' };
    } catch (error) { console.error('Logout failed:', error); this.token = null; localStorage.removeItem('webauthn-token'); return { success: false, error: error.message }; }
  }

  async getUserInfo() {
    try {
      if (!this.token) return { success: false, error: 'No token available' };
      const response = await fetch(`${this.baseURL}/api/auth/user`, { method: 'GET', headers: { 'Authorization': `Bearer ${this.token}`, 'Content-Type': 'application/json' } });
      if (response.status === 401) { this.token = null; localStorage.removeItem('webauthn-token'); return { success: false, error: 'Authentication expired' }; }
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const result = await response.json();
      return result;
    } catch (error) { console.error('Failed to get user info:', error); return { success: false, error: error.message }; }
  }

  getDeviceName() { const ua = navigator.userAgent; if (/iPhone/i.test(ua)) return 'iPhone'; if (/iPad/i.test(ua)) return 'iPad'; if (/Android/i.test(ua)) return 'Android Device'; if (/Windows/i.test(ua)) return 'Windows PC'; if (/Macintosh/i.test(ua)) return 'Mac'; if (/Linux/i.test(ua)) return 'Linux PC'; return 'Unknown Device'; }
